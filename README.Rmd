...
[![Travis build status](https://travis-ci.org/Torvaney/ggsoccer.svg?branch=master)](https://travis-ci.org/Torvaney/ggsoccer)
...
Because of the way coordinates get flipped, we must also reverse the y-axis
to ensure that the orientation remains correct.
...
If you have any other data providers that would be useful to add, please let me 
know by opening a new issue, or by sending me a message 
[on twitter](https://twitter.com/torvaney)!
...
By default, the size of the D (penalty box arc) and centre circle are set to the
distance between the penalty spot and the goalline.

Therefore, under certain conditions, the centre circle and D can appear too 
large. If you find yourself working with a data provider that has this issue, then
you can add an optional `arc_size` parameter to your pitch specification to control
this directly.

```{r example_custom_circle}
...
```{r print_custom_circle, echo=FALSE, , fig.width=4.5}
print(p1)
print(p2)
```
...
[matplotsoccer](https://github.com/TomDecroos/matplotsoccer) might be able to
help you out.
